...
export async function POST(
  ...
  try {
    const result = await withServerErrorLogging(
      ...
      {
        message: "admin_customer_approval_route_failed",
        code: "admin_customer_approval_route_failed",
        source: "api",
        area: "admin",
        path: `/api/admin/customer-approvals/${id}/approve`,
        role: "admin",
      }
    );
    ...
    return NextResponse.json(
      { error: "Unable to approve customer." },
      { status: 500 }
    );
  }
}