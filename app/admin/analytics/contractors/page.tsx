...
export default function AdminContractorAnalyticsPage() {
  ...
  async function loadPage(selectedRange: AnalyticsRange) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/analytics/breakdown?segment=contractors&range=${selectedRange}`,
        { cache: "no-store" }
      );
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Failed to load contractor analytics.");
      }
  ...
  const onboardingStarted = breakdown?.events.contractor_onboarding_started ?? 0;
  const onboardingSubmitted =
    breakdown?.events.contractor_onboarding_submitted ?? 0;
  const bidsSubmitted = breakdown?.events.submit_bid ?? 0;
  const approvalRequested =
    breakdown?.events.customer_approval_requested ?? 0;
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-[#4B5563]">
              Contractor onboarding, approval requests, and bidding activity.
            </p>
      ...
      {!loading && !err && breakdown ? (
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard label="Total contractor events" value={breakdown.totalEvents} />
            <StatCard label="Onboarding started" value={onboardingStarted} />
            <StatCard label="Onboarding submitted" value={onboardingSubmitted} />
            <StatCard label="Approval requests" value={approvalRequested} />
            <StatCard label="Bids submitted" value={bidsSubmitted} />
            <StatCard
              label="Onboarding submit rate"
              value={`${breakdown.conversions.onboardingSubmitRate ?? 0}%`}
              hint="submitted / started"
            />
            <StatCard
              label="Bid rate from onboarding"
              value={`${breakdown.conversions.bidSubmitRateFromOnboarding ?? 0}%`}
              hint="bids / onboarding submitted"
            />
          </section>
