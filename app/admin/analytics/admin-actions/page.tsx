...
export default function AdminActionsAnalyticsPage() {
  ...
  async function loadPage(selectedRange: AnalyticsRange) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/analytics/breakdown?segment=admin_actions&range=${selectedRange}`,
        { cache: "no-store" }
      );
      ...
      if (!res.ok) {
        throw new Error(json?.error || "Failed to load admin action analytics.");
      }
  ...
  const approved = breakdown?.events.admin_contractor_approved ?? 0;
  const returnedToDraft =
    breakdown?.events.admin_contractor_returned_to_draft ?? 0;
  const approvalVsReturnRate =
    breakdown?.conversions.approvalVsReturnRate ?? 0;
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-[#4B5563]">
              Approval decisions and tracked admin-side operational actions.
            </p>
      ...
      {loading ? (
        <section className="rounded-2xl border border-[#D9E2EC] bg-white p-6 shadow-sm">
          <p className="text-sm text-[#4B5563]">
            Loading admin action analytics...
          </p>
        </section>
      ) : null}
      ...
      {!loading && !err && breakdown ? (
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <StatCard label="Total admin events" value={breakdown.totalEvents} />
            <StatCard label="Approved contractors" value={approved} />
            <StatCard label="Returned to draft" value={returnedToDraft} />
            <StatCard
              label="Approval rate"
              value={`${approvalVsReturnRate}%`}
              hint="approved / approved + returned"
            />
          </section>
